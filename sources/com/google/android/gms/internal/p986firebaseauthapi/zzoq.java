...
public final class zzoq implements zzat {
    ...
    public final byte[] zza(byte[] bArr, byte[] bArr2) throws GeneralSecurityException {
        ...
        if (length >= 16) {
            ...
            cipher.init(2, new SecretKeySpec(this.zzd, KeyProvider18.KEY_ALGORITHM_AES), new IvParameterSpec(bArr4));
            byte[] copyOfRange2 = Arrays.copyOfRange(bArr, 16, length);
            byte[] doFinal = cipher.doFinal(copyOfRange2);
            if (copyOfRange2.length == 0 && doFinal == null && zzpt.zza()) {
                doFinal = new byte[0];
            }
            ...
            for (int i = 0; i <= 0; i++) {
                ...
                zza2 = zzor.zzd(zzib.zzb(zza2), this.zzc.zza(bArr6, 16));
            }
            byte[] bArr7 = bArr5[1];
            ...
            if (length2 >= 16) {
                ...
                bArr3 = zzor.zzd(zzib.zza(bArr7), zzib.zzb(zza2));
            }
            if (zzor.zzb(copyOfRange, this.zzc.zza(bArr3, 16))) {
                return doFinal;
            }
