...
public final class zzfos {
    public static int zza(int i, int i2, String str) {
        ...
        if (i < 0) {
            str2 = zzfpg.zzb("%s (%s) must not be negative", "index", Integer.valueOf(i));
        } else if (i2 < 0) {
            throw new IllegalArgumentException(C13715c.m34241g(26, "negative size: ", i2));
        } else {
            str2 = zzfpg.zzb("%s (%s) must be less than size (%s)", "index", Integer.valueOf(i), Integer.valueOf(i2));
        }
    ...
    public static void zzg(int i, int i2, int i3) {
        ...
        if (i < 0 || i2 < i || i2 > i3) {
            if (i < 0 || i > i3) {
                ...
                str = zzfpg.zzb("end index (%s) must not be less than start index (%s)", Integer.valueOf(i2), Integer.valueOf(i));
            }
    ...
    private static String zzj(int i, int i2, String str) {
        if (i < 0) {
            return zzfpg.zzb("%s (%s) must not be negative", str, Integer.valueOf(i));
        } else if (i2 >= 0) {
            return zzfpg.zzb("%s (%s) must not be greater than size (%s)", str, Integer.valueOf(i), Integer.valueOf(i2));
        } else {
            throw new IllegalArgumentException(C13715c.m34241g(26, "negative size: ", i2));
        }
    }
