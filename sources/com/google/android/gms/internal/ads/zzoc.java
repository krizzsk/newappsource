...
final class zzoc {
    ...
    public final AudioTrack zzb(boolean z, zzi zzi2, int i) throws zznn {
        ...
        try {
            int i2 = zzeg.zza;
            if (i2 >= 29) {
                AudioTrack$Builder sessionId = new AudioTrack$Builder().setAudioAttributes(zzi2.zza()).setAudioFormat(new AudioFormat.Builder().setSampleRate(this.zze).setChannelMask(this.zzf).setEncoding(this.zzg).build()).setTransferMode(1).setBufferSizeInBytes(this.zzh).setSessionId(i);
                if (this.zzc == 1) {
                    z2 = true;
                } else {
                    z2 = false;
                }
                audioTrack = sessionId.setOffloadedPlayback(z2).build();
            } else if (i2 >= 21) {
