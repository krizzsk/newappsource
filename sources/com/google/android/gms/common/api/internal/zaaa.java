...
final class zaaa implements zaca {
    ...
    public static zaaa zag(Context context, zabe zabe, Lock lock, Looper looper, GoogleApiAvailabilityLight googleApiAvailabilityLight, Map map, ClientSettings clientSettings, Map map2, Api.AbstractClientBuilder abstractClientBuilder, ArrayList arrayList) {
        ...
        for (Api api : map2.keySet()) {
            ...
            if (bVar.containsKey(zab2)) {
                bVar3.put(api, (Boolean) map3.get(api));
            } else if (bVar2.containsKey(zab2)) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (bVar3.containsKey(zat.zaa)) {
                arrayList2.add(zat);
            } else if (bVar4.containsKey(zat.zaa)) {
                arrayList3.add(zat);
            } else {
                throw new IllegalStateException("Each ClientCallbacks must have a corresponding API in the isOptionalMap");
            }
        }
        return new zaaa(context, zabe, lock, looper, googleApiAvailabilityLight, bVar, bVar2, clientSettings, abstractClientBuilder, client, arrayList2, arrayList3, bVar3, bVar4);
    }
